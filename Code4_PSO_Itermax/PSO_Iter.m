...
Gn = [1 4 8 10 20 30];          % the max iteration number
...
PAPR_PSO = zeros(length(Gn),NumSymb);    	% use to note the papr with PSO_PTS

PAPR0 = 0:0.25:12;                          % the vaule range of PAPR0

CntNoPTS = zeros(1,length(PAPR0));          % use to note the value of CCDF without PTS
Cnt_PSO = zeros(length(Gn),length(PAPR0)); 	% use to note the value of CCDF with PSO_PTS
%========================================================================================%
...
for n = 1:1:NumSymb
    ...
    %------------------------------- PAPR with PSO_PTS ----------------------------------%
    for kk = 1:1:length(Gn)
        w = wmax-(wmax-wmin)/Gn(kk)*(1:Gn(kk)); % inertia weight vector
        PAPR_PSO(kk,n) = PSO_PTS( Symbol_ifft2,W,Gn(kk),initial_w,initial_v,c1,c2,Vmax,w );
    end
    %------------------------------------------------------------------------------------%
    n
...
hold on
semilogy(PAPR0,Cnt_PSO(1,:),'-mv');
hold on;
semilogy(PAPR0,Cnt_PSO(2,:),'-b^');
hold on;
semilogy(PAPR0,Cnt_PSO(3,:),'-ks');
hold on;
semilogy(PAPR0,Cnt_PSO(4,:),'-rp');
hold on;
semilogy(PAPR0,Cnt_PSO(5,:),'-bd');
hold on;
semilogy(PAPR0,Cnt_PSO(6,:),'-kh');
grid on;
legend('without PTS','Gn = 1','Gn = 4','Gn = 8','Gn = 10','Gn = 20','Gn = 30');
xlabel('PAPR0(dB)'),ylabel('Pr(PAPR>PAPR0)');
title('Comparision of based PSO PTS with diffenert Iteration')
xlim([6 12]);
hold off
