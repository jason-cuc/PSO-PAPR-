...
Gn = 0:2:30;                    % the max iteration number
...
for kk = 1:1:length(Gn)
    ...
    for n = 1:1:NumSymb
        ...
        if ( kk == 1 )
            ...
            PAPR_PSO(kk,n) = PowerMax/PowerMean;
        end
        %--------------------------------------------------------------------------------%
...
PAPR_PSO_Iter = mean(10*log10(PAPR_PSO),2);

figure(1)
plot(Gn,PAPR_PSO_Iter,'-k*');
hold on
grid on
xlabel('Iter'),ylabel('PAPR(dB)');
ylim([7.5 10]);
title('the PAPR of OFDM system VS Iterations')
hold off
